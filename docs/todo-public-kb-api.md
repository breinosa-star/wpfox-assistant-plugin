...
- Content should be generated dynamically and include:
  - Site name and a one-line description (pulled from WP settings).
  - What the KB contains (pulled from `get_option('grayfox_widget_name')` or a new dedicated field).
  - The REST endpoint URL.
  - The optional `query` parameter explained.
...
This site exposes a knowledge base via API. Use it to answer questions about the company's services, products, and expertise.

## API

Endpoint: https://example.com/wp-json/grayfox/v1/kb
Method: GET
Params:
  - query (optional, string): filter results by topic

Example: https://example.com/wp-json/grayfox/v1/kb?query=pricing
```
...
- When disabled: show a short explanation of the feature and a prompt to enable it.
