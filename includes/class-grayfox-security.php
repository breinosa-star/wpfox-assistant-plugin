 ...
 *   2. Inspect message for injection patterns and off-topic requests.
 *   3. If flagged, add strikes (injection = +3, off-topic/length = +1).
 *   4. Apply progressive penalty:
...
class GrayFox_Security {
	...
	/**
	 * Low-severity: off-topic requests (programming help, unrelated tasks).
	 * Match adds 1 strike.
	 */
	const OFFTOPIC_PATTERNS = array(
		// Explicit code-writing requests
		'/\b(?:write|create|generate|give\s+me|show\s+me|make)\b.{0,40}\b(?:python|javascript|typescript|java|php|ruby|golang|go|rust|swift|bash|shell|sql|c\+\+|kotlin)\b.{0,40}\b(?:code|script|program|function|class|snippet|algorithm|method)\b/is',
		// Bare code fragments indicating intent to abuse the model as a coding assistant
		'/\b(?:def |import os|import sys|import re|#include\s*<|public\s+static\s+void\s+main|SELECT\s+\*\s+FROM|DROP\s+TABLE)\b/i',
		// dhondt, sainte-laguë, etc. — electoral math unrelated to business
		'/\b(?:dhondt|d\'hondt|sainte.lagu[eë]|highest.averages\s+method)\b/i',
	);
	...
	public static function inspect( string $message ): array {
		...
		// Off-topic (low severity).
		foreach ( self::OFFTOPIC_PATTERNS as $pattern ) {
			if ( preg_match( $pattern, $message ) ) {
				return array( 'score' => 1, 'reason' => 'Off-topic request detected.', 'type' => 'offtopic' );
			}
		}
	...
	public static function classify_with_llm(
		...
		// Load the last 3 messages from this session so the classifier can judge
		// the current message in context — e.g. a customer answering the assistant's
		// question about their business should never be flagged as off-topic.
		$history_context = '';
		if ( ! empty( $session_id ) ) {
			global $wpdb;
			...
			$recent_rows   = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
				$wpdb->prepare(
					"SELECT m.role, m.content FROM `{$safe_msg}` m
					 ...
					 ORDER BY m.id DESC LIMIT 3", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$session_id
				),
				ARRAY_A
			);
			...
			if ( ! empty( $recent_rows ) ) {
				...
				$history_context = "\n\nRecent conversation context (last 3 messages before this one):\n";
