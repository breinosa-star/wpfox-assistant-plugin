...
class GrayFox_Settings {
	...
	public function register( GrayFox_Loader $loader ): void {
		...
		$loader->add_action( 'wp_ajax_grayfox_verify_key', $this, 'ajax_verify_key' );
	...
	public function register_settings(): void {
		...
		register_setting( self::OPTION_GROUP, 'grayfox_license_key', array(
			'type'              => 'string',
		...
		add_settings_field(
			'grayfox_license_key',
			__( 'License Key', 'grayfox' ),
			array( $this, 'render_license_key_field' ),
			self::PAGE_SLUG,
			'grayfox_license'
		);

		register_setting( self::OPTION_GROUP, 'grayfox_license_tier', array(
			'type'              => 'string',
		...
		register_setting( self::OPTION_GROUP, 'grayfox_platform_url', array(
			'type'              => 'string',
			'sanitize_callback' => array( $this, 'sanitize_platform_url' ),
			'default'           => 'https://api.grayfox.io',
		) );

		add_settings_field(
			'grayfox_platform_url',
			__( 'Platform URL', 'grayfox' ),
			array( $this, 'render_platform_url_field' ),
			self::PAGE_SLUG,
			'grayfox_llm'
		);
	...
	public function render_license_section(): void {
		echo '<p>' . esc_html__( 'Enter your GrayFox license key. Click Verify to validate against the platform.', 'grayfox' ) . '</p>';
	}
	...
	/** Render the license key field with a Verify button. */
	public function render_license_key_field(): void {
		$encrypted = get_option( 'grayfox_license_key', '' );
		$display   = ! empty( $encrypted ) ? str_repeat( '*', 24 ) : '';
		?>
		<input type="text"
			   id="grayfox_license_key"
			   name="grayfox_license_key"
			   value="<?php echo esc_attr( $display ); ?>"
			   class="regular-text"
			   placeholder="GFX-XXXX-XXXX-XXXX"
			   autocomplete="off" />
		<button type="button" id="grayfox-verify-key" class="button button-secondary">
			<?php esc_html_e( 'Verify Key', 'grayfox' ); ?>
		</button>
		<span id="grayfox-verify-result" style="margin-left:8px;"></span>
		<p class="description">
			<?php esc_html_e( 'Your GrayFox license key.', 'grayfox' ); ?>
		</p>
		<script>
		(function() {
			var btn = document.getElementById('grayfox-verify-key');
			if (!btn) return;
			btn.addEventListener('click', function() {
				var keyInput = document.getElementById('grayfox_license_key');
				var result = document.getElementById('grayfox-verify-result');
				result.textContent = '<?php echo esc_js( __( 'Verifying...', 'grayfox' ) ); ?>';
				result.style.color = '#666';

				var data = new FormData();
				data.append('action', 'grayfox_verify_key');
				data.append('license_key', keyInput.value);
				data.append('_wpnonce', '<?php echo esc_js( wp_create_nonce( 'grayfox_verify_key' ) ); ?>');

				fetch(ajaxurl, { method: 'POST', body: data })
					.then(function(r) { return r.json(); })
					.then(function(resp) {
						if (resp.success) {
							result.textContent = resp.data.message || '<?php echo esc_js( __( 'Valid!', 'grayfox' ) ); ?>';
							result.style.color = 'green';
						} else {
							result.textContent = resp.data || '<?php echo esc_js( __( 'Verification failed.', 'grayfox' ) ); ?>';
	...
	/** Render platform URL field. */
	public function render_platform_url_field(): void {
		$value = get_option( 'grayfox_platform_url', 'https://api.grayfox.io' );
		?>
		<input type="url"
			   id="grayfox_platform_url"
			   name="grayfox_platform_url"
			   value="<?php echo esc_attr( $value ); ?>"
			   class="regular-text"
			   placeholder="https://api.grayfox.io" />
		<p class="description">
			<?php esc_html_e( 'GrayFox platform API URL. Change only if self-hosting.', 'grayfox' ); ?>
		</p>
		<?php
	}
	...
	/**
	 * Sanitize and encrypt the license key on save.
	 *
	 * @param string $input Raw input.
	 * @return string Encrypted value, or existing value on no change.
	 */
	public function sanitize_license_key( string $input ): string {
		$input = trim( sanitize_text_field( $input ) );
		if ( empty( $input ) ) {
			return get_option( 'grayfox_license_key', '' );
		}
	...
	/**
	 * Sanitize the platform URL: must be a valid https:// URL.
	 *
	 * Rejects http://, ftp://, file://, and all other schemes to prevent SSRF.
	 *
	 * @param mixed $input Raw input.
	 * @return string Validated URL, or the existing stored value on failure.
	 */
	public function sanitize_platform_url( $input ): string {
		$url = esc_url_raw( (string) $input );
		if ( empty( $url ) || 0 !== strpos( $url, 'https://' ) ) {
			add_settings_error(
				'grayfox_platform_url',
				'invalid_scheme',
				__( 'Platform URL must start with https://.', 'grayfox' )
			);
			return get_option( 'grayfox_platform_url', 'https://api.grayfox.io' );
		}
		return $url;
	}
	...
	/**
	 * AJAX handler to verify a license key against the platform.
	 */
	public function ajax_verify_key(): void {
		check_ajax_referer( 'grayfox_verify_key' );
		...
		if ( empty( $raw_key ) || preg_match( '/^\*+$/', $raw_key ) ) {
			// Use stored encrypted key.
			$stored_enc = get_option( 'grayfox_license_key', '' );
			if ( empty( $stored_enc ) ) {
				wp_send_json_error( __( 'No license key configured.', 'grayfox' ) );
			}
			$raw_key = grayfox_decrypt( $stored_enc );
		}
		...
		if ( $code >= 200 && $code < 300 && ! empty( $body['valid'] ) ) {
			...
			wp_send_json_success( array(
				...
				'message' => sprintf(
					...
					__( 'License valid. Tier: %s', 'grayfox' ),
					$tier
				),
			) );
		} else {
			wp_send_json_error( $body['message'] ?? __( 'License validation failed.', 'grayfox' ) );
		}
	}
