 ...
 *   3. Set get_tier() to the minimum license tier required ('all', 'starter', 'pro', 'enterprise').
 *
 * @package GrayFox
 */
...
abstract class GrayFox_Tool {
	 ...
	 * Use 'all' to make the tool available on every tier including free.
	 * Otherwise: 'starter', 'pro', or 'enterprise'.
	 */
	abstract public function get_tier(): string;
...
class GrayFox_Tool_Capture_Email extends GrayFox_Tool {
	...
	public function execute( array $args ): string {
		...
		return wp_json_encode( array( 'success' => true ) );
	}
}
...
class GrayFox_Tools {
	...
	private static function init(): void {
		...
		self::register( new GrayFox_Tool_Search_KB() );       // tier: all
		self::register( new GrayFox_Tool_Capture_Email() );   // tier: all

		// Future tools — uncomment and implement when ready:
		// self::register( new GrayFox_Tool_Check_Availability() ); // tier: pro
		// self::register( new GrayFox_Tool_Create_Booking() );      // tier: pro
	}

	/**
	 * Get all tools available for the given license tier.
	 *
	 * Tier hierarchy (ascending): '' → starter → pro → enterprise.
	 * A tool with tier 'pro' is available to pro and enterprise, not to '' or starter.
	 * A tool with tier 'all' is always included.
	 *
	 * @param string $tier License tier from grayfox_license_tier option.
	 * @return GrayFox_Tool[]
	 */
	public static function get_for_tier( string $tier ): array {
		self::init();

		$hierarchy  = array( '', 'starter', 'pro', 'enterprise' );
		$tier_level = array_search( $tier, $hierarchy, true );
		if ( false === $tier_level ) {
			$tier_level = 0;
		}
		...
		foreach ( self::$registry as $tool ) {
			$tool_tier = $tool->get_tier();
			if ( 'all' === $tool_tier ) {
				$tools[] = $tool;
				continue;
			}
			$required = array_search( $tool_tier, $hierarchy, true );
			if ( false !== $required && $tier_level >= $required ) {
				$tools[] = $tool;
			}
		}
	...
	/**
	 * Get OpenAI-format tool definitions for the given tier.
	...
	public static function get_definitions_for_tier( string $tier ): array {
		return array_map( static fn( $t ) => $t->get_definition(), self::get_for_tier( $tier ) );
	}
	...
	public static function execute( string $name, array $args ): string {
		...
		if ( ! isset( self::$registry[ $name ] ) ) {
			error_log( 'GrayFox Tools: unknown tool called: ' . $name );
		...
		try {
			...
			error_log( 'GrayFox Tools: error executing "' . $name . '": ' . $e->getMessage() );
